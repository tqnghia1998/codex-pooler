...
`GET /v1/models` and `GET /backend-api/codex/models` serve a hardcoded catalog (`STATIC_MODEL_CATALOG` in `src/domain.js`) and never query upstreams. Add new models by editing that constant. The catalog is advertising metadata only; it does not gate proxy routing, which still enforces per-scope and per-upstream model lists.
