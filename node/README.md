...
- adding, editing, and deleting Codex, Compass, or Claude Enterprise OAuth upstreams (Codex/Compass URLs are fixed; Claude may use an operator-supplied Anthropic-compatible `base_url`);
- reading the provider's current quota window (monthly when the provider reports one); AIS/CQP accounts are marked `ais` because their quota requires a separate Compass SSO session;
...
| HTTP ingress and errors | Bounded JSON/compressed bodies, gzip/deflate/zstd, timeouts, OpenAI-shaped errors, provider-detail redaction, bounded native-only misalignment guidance, allowlisted parameter-validation relay, sanitized upstream 429 with `Retry-After`, valid Anthropic 4xx passthrough | `test/http-ingress.test.js`, `test/http-errors.test.js`, `test/gateway-routes.test.js` |
| Pricing and settlement | OpenAI/Anthropic JSON and streaming usage, cache fields, dated model suffix pricing, requested priority pricing when Codex echoes `default`, exact ultrafast-tier pricing without fallback, authoritative `price_cost_usd`, idempotent replacement deltas | `test/pricing.test.js`, `test/domain.test.js`, `test/proxy.test.js` |
...
- Codex resolves `previous_response_id` only on the WebSocket connection that produced the earlier response. Public `POST /v1/responses` over HTTP therefore returns `400 previous_response_not_found` before dispatch; resend the full input without the anchor. A public Responses WebSocket can continue an anchor on the same live upstream connection, but a reconnect cannot recover its connection-local history. Native backend HTTP remains a direct provider route and may return sanitized `unsupported_parameter` for an anchored request.
...
runs entirely in Node; Claude requests use the Enterprise OAuth compatibility path
...
`CODEX_POOLER_CODEX_STREAM_BOOTSTRAP_BUFFERING` enables bounded pre-output SSE buffering for public Codex Responses. When enabled, the proxy holds at most the configured bytes/events/time window so an overload terminal delivered after `response.created` can still fail over before downstream output. It is disabled by default because holding headers can conflict with unusually short client timeouts.

`CODEX_POOLER_CODEX_OPTIMIZE_MULTI_AGENT_V2` and `CODEX_POOLER_CODEX_ORPHAN_DELEGATION_COMPATIBILITY` are opt-in compatibility transforms. They run only for recognized Codex user-agent patterns; User-Agent is client-controlled and is only a compatibility hint, not an authentication boundary. The former rewrites the collaboration namespace and readable agent-message content, while the latter handles unmatched `codex_app` delegation outputs when `x-openai-subagent: collab_spawn` is present. Responses are restored before returning to the client. Image-generation tool injection is intentionally not performed.
...
The gateway store uses SQLite WAL with `synchronous=FULL` and the default
automatic checkpoint policy. Common request writes serialize only the affected
upstream or accounting/pin collections, reuse prepared statements, and commit
...
POST   /api/upstreams/:id/refresh-profile  # Claude OAuth identity only
...
A `response.incomplete` naming insufficient quota, exhausted credit, or organization/project spend limits fails the turn and cools that account; a collected non-streaming response may fail over before output. Other incomplete reasons such as `max_output_tokens` remain ordinary incomplete results. A final upstream HTTP `429` stays a sanitized `rate_limit_error` with `Retry-After`, and public WebSocket 429 failures use that error type without exposing provider details. Bounded `400` validation rejections relay only allowlisted codes and field paths; Chat input paths are mapped to `messages`, while provider messages are not exposed.
...
Quota and reauthentication transitions clear ordinary session affinity so new work can route elsewhere, but committed response-continuation pins are retained and never silently moved. A failed scheduled OAuth refresh does not remove an account from routing while its saved access-token expiry remains in the future; runtime refresh failures, missing expiries, expired tokens, and reauthentication-required state remain ineligible. Credential replacement and generation-fenced attempt leases prevent stale in-flight success or failure from overwriting newer account state. Public upstream records expose only sanitized health fields. `POST /api/upstreams/:id/clear-cooldown` clears quota cooldown state without clearing reauthentication-required state or unrelated routing history.

Create a Codex upstream with `{ "type": "codex", "authJson": "..." }`, a Compass upstream with `{ "type": "compass", "projectId": "...", "projectKey": "...", "quotaSource": "compass|ais" }`, or a Claude Enterprise OAuth upstream with `{ "type": "claude", "authJson": "{\"access_token\":\"...\",\"refresh_token\":\"...\"}" }`. The API endpoints `POST /api/claude/oauth/start` and `POST /api/claude/oauth/exchange` support the Claude Code PKCE flow; the dashboard supports importing OAuth credential JSON and manual OAuth quota/profile refresh. Claude OAuth profile metadata is hydrated best-effort on create or credential replacement; manually imported records can be updated with `POST /api/upstreams/:id/refresh-profile`. Claude requests are native `/v1/messages` and `/v1/messages/count_tokens` calls; use `x-upstream-type: claude` when Compass and Claude upstreams coexist. Names are derived server-side from masked account email or a generic Claude account label. Claude credential metadata may carry CPA-compatible `cloak_mode` (`never` preserves caller placement), `cloak_strict_mode`, `cloak_sensitive_words` (array or comma-separated string), IANA `timezone`, `fingerprint_profile: "claude-code-cli"`, opt-in `rebuild_mid_system_message`, `header:<name>` overrides (including `$<incoming-header>` substitutions, a nested `headers` object, or cookies), per-auth `model_aliases` entries such as `{ "name": "claude-sonnet-4-6", "alias": "team-sonnet", "force-mapping": true }`, and `request_scoped_errors` rules with CPA actions `stop`, `stop-and-cooldown`, `continue`, or `continue-and-cooldown`. The management API also accepts CPA-style top-level Claude fields: `headers`, `models`, `excluded-models`, `proxy-url`, `prefix`, `rebuild-mid-system-message`, `disable-cooling`, `request-retry`, `request-scoped-errors`, `fingerprint-profile`, and `cloak`; `models` replaces the default catalog for that credential and supports CPA `name`, `alias`, `display-name`, `max-context-length`, `force-mapping`, and `is-compat` fields. Claude model suffixes such as `claude-sonnet-4-6(8192)`, `(high)`, `(auto)`, and `(none)` are applied before direct Messages dispatch. Aliases rewrite the upstream model and force-mapped responses use the client alias; scoped rules can stop or fail over a matching HTTP/SSE error and optionally cool the credential. These are applied only to the Claude wire transformation and never to authentication. The single Claude OAuth device identity is persisted in exported credential metadata. Bulk caps accept either `{ "target": "all|cap_reached|uncapped", "capDollars": 100 }` or quota rules such as `{ "rules": [{ "minQuotaLeft": 1000, "capDollars": 100 }] }`. Quota rules may also carry `unknownQuotaDollars`, applied as the cap for upstreams whose remaining quota is unknown; omitting it skips those upstreams, and the dashboard pre-fills $5. Bulk updates are sequential, not atomic.
...
HTTP dispatch generates retry attempts lazily from the ordered candidate plan
and uses an in-memory account-ID index for scope-checked lookups. Each attempt
...
route the initial PKCE exchange and advisory profile lookup through the
...
The priority list is empty by default; upstreams added to it (dashboard **Set priority**, or `PUT /api/upstreams/priority`) are dispatched in list order ahead of unlisted upstreams of the same type. Each listed position is a strict tier, while all unlisted upstreams share the final tier. Spending caps, account health, capability and model filters, hard pins, and circuits are applied before strategy ordering. Use `x-upstream-type: codex|compass|claude` or `x-upstream-id` to select explicitly. `x-codex-session-id` is an API-key-isolated soft upstream preference. Native Codex HTTP requests forward a validated provider `session-id` when supplied, along with `thread-id` and `x-client-request-id`. Without a valid provider session, they derive a scoped UUIDv5 `session-id` from `prompt_cache_key`, then from a local continuity alias when no cache key is available. Public `/v1` Codex HTTP and Responses WebSocket requests with a valid `prompt_cache_key` derive a scoped UUIDv5 `session-id` upstream; a WebSocket reconnects when that key changes. Raw aliases and derived IDs are not logged. Each pin rotates after $5 of settled, priced spend: the next ordinary request excludes the prior upstream once, then applies priority routing among the remaining eligible upstreams (or safely falls back to the same one when it is the only candidate). Thus priority `[A, B]` alternates sessions in $5 chunks: A to B to A to B, while both remain eligible. Response continuations retain their response pin. Codex Chat Completions are translated to Responses upstream and converted back, while Compass requests are sent directly to `/chat/completions`, `/responses`, or `/messages`; Claude forwards native `/messages` and `/messages/count_tokens`. Claude Enterprise OAuth requests use the Claude Code identity and protocol profile. Requests default `anthropic-version` to `2023-06-01` and relay validated response/rate-limit headers.

Codex `response.incomplete` terminals reporting exhausted credit or organization/project spend limits are failures, not successful truncations. The affected account enters quota cooldown; a pre-output HTTP turn can fail over, while already visible output is not replayed. Failed turns do not settle successful usage.
