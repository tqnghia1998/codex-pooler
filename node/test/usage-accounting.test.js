...
test('serves scoped settled gateway usage and rejects usage filters', async () => {
  ...
  try {
    ...
    assert.deepEqual(await usage.json(), { request_count: 1, total_tokens: 10, cached_input_tokens: 2, total_cost_usd: 3.45, total_cost_status: 'priced', limits: [], upstream_limits: [{ limit_type: 'credits', limit_window: '5h', max_value: 120, current_value: 12, remaining_value: 108, model_filter: null, source: 'upstream_usage' }] });
